...
6. **手順3で確認対象の便が1つ以上あり、実際にFlightradar24等で照合できた場合は、値が変化していなくても `updatedAt` を現在の日本時間に更新する。** `source` に実際に読んだサイト名を書く。
   - **重要（要注意・過去に発生したバグ）**: index.html は `updatedAt` が45分より古いと画面に「⚠️少し前のデータです」と表示する。手順3で対象便があるのに照合結果が前回と同じだからといって `updatedAt` を据え置くと、実際には毎回チェックしているのに家族には「データが古い」と見え続けてしまう。**対象便がある回は、値の変化の有無にかかわらず必ず `updatedAt` を更新してcommit & pushする。**
   - 手順3で対象便が0件だった回（まだ誰も確認対象外）は、何もせず終了してよい（`updatedAt` はそのままでよい）。
7. JSON として正しいか確認する（`python3 -m json.tool status.json` など）。
8. commit & push する。
   - legの中身（`updatedAt`以外）が変わった回のコミットメッセージ例: `status: QF77 departed 23:31 AWST`
   - 値は変わらず`updatedAt`だけ更新した回のコミットメッセージ例: `status: recheck QF77 (no change) 23:06 JST`
   - 手順3で対象便が0件だった回は commit しない。
9. 最後に1行で結果を報告する（例: `QF77 出発済み 23:31、SIN 着見込み 04:40` または `QF77 変化なし、再チェック済み`）。
